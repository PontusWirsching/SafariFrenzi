...
public class GemBlue extends Collectable {

	public GemBlue(float x, float y, float w, float h) {
		super(x, y, w, h);
		...
		setTexture(Resources.get("gem:blue_shadow"));
	...
	class GemSprite extends Sprite {
		...
		public GemSprite(String id, float x, float y, float width, float height) {
			...
			setTexture(Resources.get("gem:blue"));
		...
		public void draw(SpriteBatch sb) {
			...
			if (Util.getDistance(x, y, Game.gui.get("chest").x, Game.gui.get("chest").y) < 20) {
				LevelHandler.getSelected().score += Values.GEM_BLUE;
				Money.add(Values.GEM_BLUE);
